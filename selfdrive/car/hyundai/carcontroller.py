...
class CarController():
  ...
  def update(self, enabled, CS, frame, CC, actuators, pcm_cancel_cmd, visual_alert,
    ...
    str_log1 = 'CV={:03.0f}  TQ={:03.0f}  ST={:03.0f}/{:01.0f}/{:01.0f}  R={:03.0f}'.format(abs(self.model_speed), abs(new_steer), self.steerMax, self.steerDeltaUp, self.steerDeltaDown, self.timer1.sampleTime())
    ...
    if ((self.params.get('LimitSetSpeedCamera') is not None and self.params.get('LimitSetSpeedCamera') != "0") or int(self.params.get('OpkrSafetyCamera')) == 1) and not CS.acc_active:
