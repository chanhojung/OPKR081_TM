...
class Spdctrl(SpdController):
    ...
    def update_lead(self, sm, CS, dRel, yRel, vRel):
        ...
        # 선행차량이 멀리 있는 상태에서 감속 조건
            ...
            #    lead_wait_cmd, lead_set_speed = self.get_tm_speed( CS, 15, 5)
                ...
                lead_wait_cmd, lead_set_speed = self.get_tm_speed( CS, max(15, 150+(lead_objspd*3.6*10)), -1)
    ...
    def update_log(self, CS, set_speed, target_set_speed, long_wait_cmd ):
        ...
        str3 = 'VL={:03.0f}/{:03.0f}  TS={:03.0f}  TM={:03.0f}/{:03.0f}  MODE={:s}'.format(set_speed, CS.VSetDis, int(round(self.target_speed)), long_wait_cmd, self.long_curv_timer, self.steer_mode )
